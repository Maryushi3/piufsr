...
Every pixel/pattern command is answered with `Panel <p> OK` or
`Panel <p> FAIL`, followed by the `> ` interactive prompt.
...
```
./ledmaker.py /dev/ttyACM0 --identify
```
...
`PANEL_LAYOUTS` line to paste into the script. Wrong layout = wrong preview,
but saved patterns still render correctly on the panel itself (they are
stored in the panel's own index space).
