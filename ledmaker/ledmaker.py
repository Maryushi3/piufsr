...
REPLY_TIMEOUT = 0.5    # max seconds to wait for one reply
MAX_RETRIES = 3
...
def clear_panel(ser, panel, delay, retries):
    ...
    for y in range(PANEL_SIZE):
        ...
        for x in range(PANEL_SIZE):
            if not set_pixel(ser, panel, x, y, False, delay, retries):
    ...
    try:
        ...
        for y in range(PANEL_SIZE):
            for x in range(PANEL_SIZE):
                ...
                if not set_pixel(ser, panel, x, y, True, args.delay, args.retries):
                ...
                if not set_pixel(ser, panel, x, y, on, args.delay, args.retries):
        ...
        if send_command(ser, f"w {panel} {slot}", args.delay, args.retries):
            print("Saved.")
        else:
